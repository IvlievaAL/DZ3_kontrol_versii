...
* далее **надо представиться гиту** (вроде как один раз): *git config --global user.name* "Имя пользователя" и  *git config --global user.email* почта. Если потом забудешь, как именно представлялась - это покажет команда *git config --list*
...
* команда *git add имяфайла.расширение* снова. ВНИМАНИЕ: **если** в этой папке гит **отслеживает несколько файлов**, то внесенные в каждый из них изменения будут закоммичены только в том случае, если каждый из этих файлов был добавлен в индекс этой командой (или общей для всех с флагом *-a* - см.ниже) - даже если изменения были в них сохранены обычным путем.
...
* и сделать коммит с комментарием о внесенных изменениях: *git commit -m"комментарий"* или, вариант с флагом, *git commit -a -m"комментарий"*. ВНИМАНИЕ: флаг *-a* означает указание гаком добавить все отслеживаемые гитом в этом репозитории файлы в индекс - т.е. приведет **к созданию коммита во всех** отслеживаемых в данном репозитории файлах. Если этого не нужно, то флаг не ставить. Забавная, кстати, штука может помочь всякий раз не добавлять файлы в индекс вручную, если их действительно нужно все коммитить каждый раз - создать в репозитории спец.файл *.gitkeep*, прописать в нем, по 1 на строку, имена нужных файлов, сохранить файл и закоммитить его - это указание гиту, что перечисленные файлы надо добавлять в каждый коммит.
...
![Здесь находится мем4](image4.jpg)
...
! Хинт: можно создать ветку и не от той, на которой ты сейчас находишься: *git branch имяновойветки имяветкиоткоторойпойдетновая* Переключения на свежесозданную ветку при этом не произойдет!
...
## История изменений в гите и что с ней можно делать
![Здесь находится мем2](image2.jpg)

ВНИМАНИЕ! **Изменения истории приводят к проблемам при слиянии** веток (т.е. 100% - при использовании удаленных репозиториев), поэтому при совместной работе над проектом откат лучше вовсе не делать!

**Внимание, чтобы выйти из режима журнала в обычный режим, надо нажать на клавишу q** (обнаружено, что ею же выходить надо из спецрежимов команд *git diff*, *git help*, *git reflog*). 

* Посмотреть, **какие коммиты и сколько их** было сделано, можно вызовом журнала изменений *git log* (или его компактной версии *git log --oneline* -покажет только коммент к каждому коммиту: так читать список удобнее, если коммитов много). При наличии нескольких веток (см.ниже) полезно набрать *git log --graph*: журнал высветится с вертикальной схемой ветвления.
...
* **Откатить** файл и гит лог **до определенного коммита**: *git reset номеркоммита* Однако рефлог продолжает помнить откаченное и тот факт, что откат был совершен. Поэтому если хочешь вернуть обратно: *git reset номероткаченногокоммитавозьмиврефлоге* - коммит восстановится, в т.ч. в гит логе.

* **Отменить** те **изменения, что были зафиксированы** в данном коммите: *git revert номеркоммита* Для гита это будет выглядеть не как удаление упомянутого коммита, а как создание еще одного, нового, коммита, в котором прописано "отменить коммит такой-то". Команда выкинет в режим редактора Vim: набрать !wq ("записать и выйти").

* Загадочная команда *git restore* "восстанавливает файлы рабочего дерева": это **сброс** состояния файла **до состояния последнего коммита** (т.е. сброс всех внесенных в него, но еще не закоммиченных, изменений). Так можно отменить добавление изменений в индекс гита, то бишь уже сделанный *git add*: *git restore --staged имяфайла*
* **Исправить последний коммит**: *git commit --amend* Подлить в него изменения: сначала *git add* их, потом эта команда. Внимание: для гита это будет выглядеть как создание еще одного коммита, "тот же, но со звездочкой", head будет теперь на котором. Если надо **изменить только комментарий** к коммиту, в индекс НЕ добавлять файл, а только *git commit --amend -m"Новый комментарий"*

![Здесь находится мем5](image5.jpg)
